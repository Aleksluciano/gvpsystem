...
export class CongregationsService {
  ...
  createCongregation(congregation: Congregation) {
    ...
    this.http
      .post<{ message: string; congregation: any }>(
        BACKEND_URL,
        congregation
      ).pipe(
        map(responseData =>
  ...
  getCongregationsServer() {
    ...
    this.http
      .get<{ message: string; congregations: any[]}>(
        BACKEND_URL
      )
      .pipe(
        map(responseData => {
          return {
            congregations: responseData.congregations.map(congregation =>
              <Congregation>{
                id: congregation._id,
  ...
  this.http
  .put(BACKEND_URL + id, congregation)
  .subscribe(responseData => {
